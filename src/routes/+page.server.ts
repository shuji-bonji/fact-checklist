...
export const load: PageServerLoad = async ({ cookies, locals, url }) => {
  ...
  const pathLang = url.pathname.split('/')[1] as LanguageCode | undefined;
  const cookieLang = cookies.get('language') as LanguageCode | undefined;
  const browserLang = (locals as { language?: LanguageCode }).language; // hooks.serverから
