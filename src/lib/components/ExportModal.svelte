...
<script lang="ts">
		...
		try {
			...
			try {
				...
				if (exportOptions.sectionBreaks) {
					// セクションごとにページを分割
					await renderSectionedPDF(pdf, tempContainer, pageWidth, pageHeight);
				} else {
					// 通常のレンダリング
					await renderContinuousPDF(pdf, tempContainer, pageWidth, pageHeight);
				}
	...
	async function renderSectionedPDF(pdf: any, container: HTMLElement, pageWidth: number, pageHeight: number) {
		...
		if (headerSection) {
			await addSectionToPDF(pdf, headerSection, pageWidth, pageHeight, false);
		}
		...
		for (let i = 0; i < sections.length; i++) {
			...
			await addSectionToPDF(pdf, sections[i] as HTMLElement, pageWidth, pageHeight, true);
		}
		...
		if (notesSection) {
			pdf.addPage();
			await addSectionToPDF(pdf, notesSection, pageWidth, pageHeight, true);
		}
	}
